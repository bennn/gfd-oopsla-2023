...
Finally, as a programmer applies an optimistic strategy, configurations along
...
a bad sign (\pct{10} of configurations succeed despite one setback) and two
...
conclusions. The data for one configuration is rather limited due to time
constraints, with 8 performance measurements and 1 run for each profiler.
A small improvement would be to run the profiler several times and choose
a ``median'' result.
