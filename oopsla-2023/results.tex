...
boundary profiling, while configurations that suffer from shallow
...
Racket ships with tools for statistical~[CITE] and boundary
profiling~\cite{}.
The boundary profiler is aware of contracts as a default feature.
...
Second, the modified boundary profiler groups contracts
...
boundary profile output, and statistical profile output.
...
\begin{enumerate}
  \item
    ...
    \code{time} function.
  \item
    Install the modified boundary profiler and run it once,
...
Most profiler results (boundary and statistical) came from \machinename{m510} machines
...
All told, \emph{optimistic, boundary} navigation is the most likely to
