...
no other users ran jobs during our reservation time.
...
of Racket (v8.6.0.2) and the Typed Racket that ships with it.
The expections are \bmname{dungeon} and \bmname{morsecode},
...
The measurements confirm that the GTP Benchmark are useful for
...
configuration are interesting starting points for the experiment.
...
\begin{itemize}
  \item
    ...
    profilers, both of which rarely succeed.
  ...
  \item
    ...
    With \emph{statistical (total)} and \emph{statistical (self)} profilers,
...
count strict successes when $T = 3$ is acceptable.  The
...
They succeed in an additional \pct{10} of
scenarios under strict success, and the $N$-loose results improve along
the same lines.  With statistical profilers, the optimistic strategies
improve slightly.

Unfortunately, the conservative strategies perform no better than before.
...
succeeded in the 3x sense in a small number of hopeless scenarios that
