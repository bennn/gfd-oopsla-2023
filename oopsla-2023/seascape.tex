...
because of the enforcement of types.
Among the several approaches to the sound enforcement of
gradual types that do not limit
...
checks.} the two leading ones are deep and shallow checks:
\begin{itemize}
  \item
    Deep checks enforce types with higher-order contracts that monitor the
    ...
    allocation.  If there are few boundaries, however, then deep checks
    impose few costs and type-driven optimizations may exceed the
    performance of untyped code~\cite{gtnffvf-jfp-2019}.

  \item
    Shallow checks enforce types with tag-level assertions injected at
    ...
    inexpensive, but the lack of proxies blurs the lines between typed and
...
for the statistical profiler. In contrast, the case for shallow's assertion should be
the opposite, meaning the statistical profiler may uncover 
costs that the boundary profiler misses.
...
\begin{itemize}
  \item
    ...
    derived from the type, before it crosses over to untyped code.  The
    proxy checks that untyped calls fully match the type by traversing
    ...
    directly to proxies.  Conversely, the statistical profiler seems less
    likely to be useful because it breaks down cost by module; however, it
    may indirectly produce useful information if the proxy slows
    down a module that calls \code{avg}.
  \item
    With shallow checks, the compiler injects in the definition of \code{avg} a test for
    ...
    Because there are no contracts in the shallow version, only simple checks, the
