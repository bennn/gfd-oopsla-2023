...
\noindent{}With deep checks, the function gets wrapped in a proxy before
it crosses over to untyped code. The proxy checks that untyped calls
fully match the type by traversing input lists.
The boundary profiler attributes costs directly to the proxy.
The statistical profile attributes costs to modules, but it may
\emph{indirectly} discover the proxy if proxy checks cause a module
to slow down.
...
There are no contracts in the shallow version, so the boundary profiler
cannot see any costs whatsoever.
The statistical profiler is in a much better position because costs
arise from extra code in each function.
