...
\begin{itemize}
  \item
    ...
    between \code{avg} and its untyped clients, which is
    derived from the type, before \code{avg} crosses over to a client.  The
  ...
  \item
    With shallow types, the compiler injects in the definition of \code{avg} a test for
...
figure~\ref{f:fsm-code:a} where \code{main} has deep types.
