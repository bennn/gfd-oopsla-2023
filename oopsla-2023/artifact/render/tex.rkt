...
(define-runtime-path tex-dir "../tex")
  ...
  (define row*
    (for/list ((rr (in-list (cdr tbl))))
      (list (bmname (first rr))
            ...
            (rev-pct (second rr)))))
  (with-output-to-file
    (build-path tex-dir "baseline-trouble.tex")
    #:exists 'replace
    (lambda ()
    ...
    (for/list ((bb (in-list (file->value (build-path data-dir "success-res.rktd")))))
      ...
      (map
        ...
        (list (bmname bm)
              ...
              pstr))))
  (with-output-to-file
    (build-path tex-dir "blackhole.tex")
    #:exists 'replace
    (lambda ()
      (tex-table (cons title* row*))))
  (void))
...
(define (f:strategy-overall #:hope? [hope? #f] #:split-bm? [split-bm? #false])
  ...
  (for ((tgt-bm (in-list (if split-bm? (map car (cdr (car stbl))) '(#f)))))
    ...
    (define out-dir (if split-bm? (let ((dd (build-path tex-dir "sky"))) (ensure-dir dd) dd) tex-dir))
...
(define (f:head2head #:all? [all? #f] #:split-bm? [split-bm? #f])
  ...
  (define out-dir
    (if split-bm?
      (let ((dd (build-path tex-dir "h2h"))) (ensure-dir dd) dd)
      tex-dir))
