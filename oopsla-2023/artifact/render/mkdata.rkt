...
(define-runtime-path trail-dir "../rational-experiment/out")
(define-runtime-path data-dir "./figure-data")
...
(define (go-h2h)
  ...
  (with-output-to-file+ (format "~a/h2h.rktd" data-dir) #:exists 'replace (lambda () (pretty-write res**)))
  (void))
...
(define (go-success #:scene scene)
  ...
  (with-output-to-file+ (format "~a/success-res.rktd" data-dir) #:exists 'replace (lambda () (pretty-write res**)))
  #;(let ()
    (define xy** (map go-xy bm**))
    (with-output-to-file (format "~a/success-xy.rktd" data-dir) #:exists 'replace (lambda () (pretty-write (map (lambda (x) (map car x)) xy**))))
    (for* ((bmxy (in-list xy**))
           (rr (in-list bmxy)))
      (define bm (first (car rr)))
      (define ss (second (car rr)))
      (define mm (third (car rr)))
      (with-output-to-file (format "~a/success-xy-~a-~a-~a.rktd" data-dir bm ss mm)
                           #:exists 'replace (lambda () (pretty-write (cadr rr))))))
  (define tbl* (combine res**))
  (with-output-to-file+
    (format "~a/success-tbl-~a.rktd" data-dir scene)
