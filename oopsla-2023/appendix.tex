...
The following nine benchmarks required changes:
...
\begin{itemize}
  ...
  \item
    ...
    all confirm the superiority of the optimistic feature-specific strategy.

  \item
    Both \bmname{morsecode} and \bmname{lnm} do better with statistical profiles
    than with feature-specific profiles.
    ...
    Curiously, statistical (total) is typically better than statistical (self) in
...
\Cref{f:h2h:bm} compares the \emph{optimistic, feature-specific} strategy
...
\begin{itemize}
  \item
    ...
    empty because no strategy succeeds on any scenario.
  \item
    The plot for \bmname{acquire} is nearly empty, again because successes are rare.
    For scenarios in which one strategy succeeds, the others are likely to succeed too.
    However, the conservative and configuration-aware strategies are slightly more
    successful than the rest with feature-specific profiles.
  \item
    Because feature-specific profiling tends to get stuck in \bmname{morsecode}
    ...
    But \emph{random boundary} also does well and even beats statistical in
    \bmname{morsecode}.
    This unexpectedly high success of random suggests that profiling is not needed
