...
and variational typing to predict the approximate performance of
configurations in a migration lattice.  This approach is promising because
it avoids the need to profile mixed-typed programs; the proposed tool
analyzes source code to identify configurations with \emph{relative} good
performance compared to the rest of the lattice. Unlike the profiler-based
strategies of this paper, the tool does not handle a full-featured type
system (with universal and union types). Moreover, due to the relative
character of static cost semantics, it does not identify
successful migration paths as described in~\Cref{sec:experiment}.
